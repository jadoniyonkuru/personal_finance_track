...
// ─── Individual savers ─────────────────────────────────────────────────────────
export function saveTransaction(data, tx) {
    const updated = {
        ...data,
...
export function saveBudget(data, budget) {
    const exists = data.budgets.findIndex((b) => b.id === budget.id);
    const budgets = exists >= 0
...
export function savePot(data, pot) {
...
export function deletePot(data, id) {
    const updated = {
        ...data,
...
export function saveRecurringBill(data, bill) {
    const exists = data.recurringBills.findIndex((b) => b.id === bill.id);
    const recurringBills = exists >= 0
        ? data.recurringBills.map((b) => (b.id === bill.id ? bill : b))
        : [...data.recurringBills, bill];
    const updated = { ...data, recurringBills };
    saveData(updated);
    return updated;
}
export function deleteRecurringBill(data, id) {
    const updated = {
        ...data,
